...
// Software Serial ports Rx and Tx are opposite to the HC12 Rx and Tx pins
...
void setup() {
  ...
  HC12ReadBuffer.reserve(128);    // Reserve 64 bytes for Serial message input
...
void loop()
{
  ...
  if (HC12End) {    // If HC12End flag is true
    ...
    Serial.println("+!");
    for (int k=4;k>=0;k--){ //Sends 5 ACK messages to sender station
      HC12.print("ACK");                  // Reply with "ACK+same message as received+!"
      HC12.print(HC12ReadBuffer);         // Reply with "ACK+same message as received+!"
      HC12.println("!");                  // Reply with "ACK+same message as received+!"
    }
    ...
    if (!HC12ReadBuffer.compareTo(passaON)){
      ...
      digitalWrite(garageControlPin,RelayOff);
      presentationBlink();
      Serial.println("Match ok! Opening Only Garage and Switching ON ledPin!!!!");
      ...
      digitalWrite(garageControlPin,RelayOff);
      presentationBlink();
      Serial.println("Match ok! Opening Only Garage and Switching OFF ledPin!!!!");
      digitalWrite(ledPin,ledStatus);
